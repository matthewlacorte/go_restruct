    ...
    def printme(self):
        try:
            ...
            for f in self.fields:
                if len(f.name) <= 10:
                    print('\t' + str(f.offset) + '\t' + f.name + '\t\t' + types[f.type])
                else:
                    print('\t' + str(f.offset) + '\t' + f.name + '\t' + types[f.type])
        except:
...
def getFieldOffset(field):
    ...
    clearCodeUnits(field.offset_address, field.offset_address.add(0x8), False)
    createData(field.offset_address, ghidra.program.model.data.IntegerDataType(), 4)
    
    field.offset = getDataAt(field.offset_address).getValue()
    print(field.offset)
...
def getFields(orig_structs):
    ...
    structs = getFieldOffsets(structs)
