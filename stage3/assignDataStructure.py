...
def createDataStructure(struct, dataTypeManager, category_path):
    ...
    try:
        ...
        # For every field in the struct...
        for field in struct.fields:
            # print(field.name + ' - ' + str(field.offset) + ' - ' + hex(field.type).strip('L'))
            ...
            if field.type == 0:
                ...
                test_struct.replace(field.offset, un4, 1, field.name+'_real', field.name + '_real')
                test_struct.replace(field.offset+4, un4, 1, field.name+'_imag', field.name + '_imag') # 2-piece
            elif field.type == 0x10: # complex128
                test_struct.replace(field.offset, un8, 1, field.name+'_real', field.name + '_real')
                test_struct.replace(field.offset+8, un8, 1, field.name+'_imag', field.name + '_imag') # 2-piece
            ...
            # TODO : Do all the below
                ...
                if REG_AX == 'EAX':
                    test_struct.replace(field.offset, string, 1, field.name+'_ptr', 'test comment - ' + field.name + '_ptr')
                    test_struct.replace(field.offset+8, int32, 1, field.name+'_len', 'test comment - ' + field.name + '_len') # 2-piece
                else:
                    test_struct.replace(field.offset, string, 1, field.name+'_ptr', 'test comment - ' + field.name + '_ptr')
                    test_struct.replace(field.offset+8, int64, 1, field.name+'_len', 'test comment - ' + field.name + '_len') # 2-piece
...
# Find struct variable assignment
def find_struct_var_ass(struct, call, dataTypeManager):

    # Get CALL runtime.newobject() instructtion
    instr = currentProgram.getListing().getInstructionAt(call)
    func = currentProgram.getListing().getFunctionContaining(instr.getAddress())
    callDepther = ghidra.app.cmd.function.CallDepthChangeInfo(func)

    # if 'main.main' not in func.getName():
    #     return
    
    # print
    # print('================================================================')
    # print('finding var for ' + str(call))
        ...
        while True:
            if instr.getMnemonicString() == 'MOV':
                if instr.getDefaultOperandRepresentation(1) == REG_AX:
                    # print('found it ' + str(instr.getAddress()) + ' - ' + str(instr))
                    if 'SP + ' in instr.getDefaultOperandRepresentation(0):
                        ...
                        # print('var offset : ' + hex(var_offset))

                        vars = func.getLocalVariables()
                        depth = callDepther.getDepth(instr.getAddress())
                        # print('Depth is ' + hex(depth))
                        val = (-1 * depth) - var_offset
                        # print('val to look for is : ' + hex(val))
                        
                        for var in vars:
                            # print(var.getName() + ' - ' + hex(var.getStackOffset()))
                            if var.getName().endswith('_' + hex(val).split('x')[-1]):
                                # print('this is the match : ' + var.getName())
...
def assignStructs(structs, struct_calls):

    print
    print('[+] Defining and assigning custom data structures')
    dataTypeManager = currentProgram.getDataTypeManager()
    
    # Create category to hold all custom data structures
    ...
    # print('associating data structure...')
    ...
    for call in struct_calls:
        ...
        if struct_calls[call].data_structure is not None:
            ...
            cnt += find_struct_var_ass(struct_calls[call], call, dataTypeManager)
...
def main():
    ...
    assignStructs(structs, struct_calls)
